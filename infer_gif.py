...
try:
    ...
    pass
print(f'GIF: {len(raw_frames)} кадров, размер {raw_frames[0].shape[:2]}')

# Model — same as demo.py with mvit32-2.onnx config
session = ort.InferenceSession('/Users/artemijmosin/Documents/projects/slovo/mvit32-2.onnx')
...
print(f'Модель: window_size={window_size}, frame_interval={frame_interval}')
print()
...
# Sliding window — точная копия логики Runner из demo.py
tensors_list = []
prediction_list = ['---']
frame_counter = 0
...
for frame_idx, frame in enumerate(raw_frames):
    ...
    if len(tensors_list) >= window_size:
        ...
        if gloss != prediction_list[-1] and gloss != '---':
            ...
            print(f'  кадр ~{frame_idx:3d}  →  "{gloss}"  conf={conf*100:.1f}%')

        # clear window like demo.py
        for _ in range(window_size):
            if tensors_list:
                tensors_list.pop(0)

print()
print('Итоговая последовательность:', ' | '.join(prediction_list[1:]))
