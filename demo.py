...
class Runner:
    ...
    def __init__(
        ...
        """
        # File playback is noticeably smoother when inference is offloaded.
        self.multiprocess = mp or bool(video_path)
        if bool(video_path) and not mp:
            logger.info("Включаю multiprocessing автоматически для видеофайла")
        self.cap = cv2.VideoCapture(video_path if video_path else 0)
        self.is_file_source = bool(video_path)
        src_fps = float(self.cap.get(cv2.CAP_PROP_FPS) or 0.0)
        # Keep natural playback pace for file input.
        self.wait_ms = 10
        if self.is_file_source and src_fps > 1.0:
            self.wait_ms = max(1, min(100, int(round(1000.0 / src_fps))))
        ...
        try:
            while self.cap.isOpened():
                if self.recognizer.started:
                    ...
                    condition = cv2.waitKey(self.wait_ms) & 0xFF
...
def parse_arguments(params: Optional[Tuple] = None) -> argparse.Namespace:
    ...
    parser.add_argument("-f", "--file", required=False, type=str, default=None, help="Path to input video file")
...
if __name__ == "__main__":
    ...
    runner = Runner(conf.model_path, conf, args.mp, args.verbose, args.length, args.file)
    runner.run()
